...
// ── Stimulus video ─────────────────────────────────────────────────────────────
// Place your video at: src/assets/stimulus_video.mp4
...
export function RecordingScreen({ navigation }: Props) {
  ...
  // ── expo-video player ────────────────────────────────────────────────────────
  const player = useVideoPlayer(STIMULUS_VIDEO, (p) => {
    p.loop   = true;
    p.volume = 0.85;
    p.muted  = false;
  });
  ...
  const pipBorderColor = pipBorderAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['rgba(79,142,247,0.3)', 'rgba(79,142,247,0.95)'],
  });
  ...
  const beginRecording = () => {
    ...
    setPhase('recording');
    // Seek to start and play stimulus video
  ...
  return (
    <View style={styles.root}>
      ...
      {/* ═══════════════════════════════════════════════
          LAYER 1 — STIMULUS VIDEO (child watches this)
          ═══════════════════════════════════════════════ */}
      <View style={StyleSheet.absoluteFill}>
        ...
        {(phase === 'idle' || phase === 'countdown') && (
          <View style={styles.placeholder}>
            ...
            <Text style={styles.placeholderSub}>
              The video will play automatically when the session begins.{'\n'}
              Position the child so their face is visible in the camera.
            </Text>
          </View>
        )}
        ...
        <VideoView
          ...
          style={[
            ...
            (phase === 'idle' || phase === 'countdown') && styles.hidden,
          ]}
          contentFit="cover"
      ...
      {/* ═══════════════════════════════════════════════
          LAYER 2 — ANALYSIS CAMERA (hidden, 1×1 px)
          ═══════════════════════════════════════════════ */}
      <Camera
      ...
      >
        ...
        {phase === 'countdown' && (
          <View style={styles.countdownOverlay}>
            <Animated.Text style={[styles.countdownNum, { transform: [{ scale: countdownScale }] }]}>
              {countdown}
            </Animated.Text>
        ...
        <Animated.View style={[styles.topHUD, { opacity: controlsOpacity }]}>
          <SafeAreaView>
            <View style={styles.topRow}>
              ...
              {(phase === 'recording' || phase === 'paused') && (
                <View style={[styles.timerBadge, phase === 'paused' && styles.timerBadgePaused]}>
                  <View style={[styles.timerDot, phase === 'paused' && styles.timerDotPaused]} />
                  <Text style={styles.timerText}>{timeStr}</Text>
                </View>
              )}

              {/* PiP — parent checks child is in frame */}
              {(phase === 'recording' || phase === 'paused') && (
            ...
            <View style={styles.idleBox}>
              <View style={styles.idleCard}>
                ...
                <Text style={styles.idleBody}>
                  • Sit the child comfortably facing the phone{'\n'}
                  • The stimulus video plays automatically{'\n'}
                  • Session runs for 3 minutes{'\n'}
                  • Watch the top-right camera to keep child in frame
                </Text>
              </View>
...
function MetricPill({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <View style={[pill.wrap, { borderColor: `${color}40` }]}>
      ...
      <Text style={[pill.value, { color }]}>{Math.round(value * 100)}%</Text>
    </View>
  );
}
const pill = StyleSheet.create({
  wrap:  { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(10,14,26,0.82)', paddingHorizontal: 9, paddingVertical: 5, borderRadius: 99, borderWidth: 1 },
...
const styles = StyleSheet.create({
  ...
  hidden: { opacity: 0 },
  placeholder: { ...StyleSheet.absoluteFillObject, backgroundColor: Colors.bg0, alignItems: 'center', justifyContent: 'center', gap: Spacing.md },
  ...
  topRow:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing.md, paddingTop: Spacing.sm, paddingBottom: Spacing.sm },

  circleBtn:     { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(0,0,0,0.6)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)' },
  circleBtnText: { fontSize: 15, color: '#fff' },

  timerBadge:       { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(0,0,0,0.65)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 99, borderWidth: 1, borderColor: 'rgba(239,68,68,0.5)' },
  ...
  pipWrapper: { width: 68, height: 90, borderRadius: 8, overflow: 'hidden', borderWidth: 2 },
  pipCamera:  { width: '100%', height: '100%' },
  pipTag:     { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.55)', alignItems: 'center', paddingVertical: 2 },
  pipTagText: { fontSize: 9, fontFamily: 'Inter_600SemiBold', color: 'rgba(255,255,255,0.7)', letterSpacing: 0.5 },

  progressTrack: { height: 3, backgroundColor: 'rgba(255,255,255,0.1)', marginHorizontal: Spacing.md, marginTop: 4, borderRadius: 99, overflow: 'hidden' },
  progressFill:  { height: '100%', backgroundColor: Colors.accent, borderRadius: 99 },

  countdownOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(4,6,9,0.78)', alignItems: 'center', justifyContent: 'center' },
  ...
  completeOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(4,6,9,0.88)', alignItems: 'center', justifyContent: 'center', gap: Spacing.sm },
  ...
  bottomArea:  { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: Spacing.md, paddingBottom: 44, gap: Spacing.sm },
  metricsRow:  { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4, flexWrap: 'wrap' },
  frameCount:  { ...Typography.caption, color: 'rgba(255,255,255,0.28)', marginLeft: 'auto' },

  idleBox:          { gap: Spacing.md },
  idleCard:         { backgroundColor: 'rgba(10,14,26,0.88)', borderRadius: Radii.md, padding: Spacing.md, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  ...
  startBtnGradient: { paddingVertical: Spacing.md + 4, alignItems: 'center', justifyContent: 'center' },
  startBtnText:     { ...Typography.title, color: '#fff' },

  controlRow:  { flexDirection: 'row', gap: Spacing.sm },
  controlBtn:  { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: Spacing.sm, backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: Radii.md, paddingVertical: Spacing.md, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  stopBtn:     { backgroundColor: 'rgba(239,68,68,0.15)', borderColor: 'rgba(239,68,68,0.3)' },
  ...
  errorScreen: { flex: 1, backgroundColor: Colors.bg1, alignItems: 'center', justifyContent: 'center', gap: Spacing.md },
